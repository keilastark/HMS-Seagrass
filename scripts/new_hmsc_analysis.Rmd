...
spat <- data.frame(spat = sprintf('spatial_%.2d',1:102)) #spatial factor column for studyDesign
...
hM <- Hmsc(Y = Y, XData = X, XFormula = ~eelgrass_lai + temprange + poly(tempmean, degree = 2) + nitratemean + curveloc + salinitymean + dissoxmean + macroalgae_biomass + detritus_biomass + shoot_density, studyDesign = studyDesign, ranLevels = list(Quadrat = rL1, Site = rL2, Region = rL3, spatial = rL4), distr = "poisson")
 
# Run MCMC chains. took at least 12 hours on KS laptop

mod <- sampleMcmc(hM, samples = 200 , transient = 100, thin = 100, verbose = 10)
...
preds1 <- computePredictedValues(mod)


MF <- evaluateModelFit(hM = mod, predY = preds)

#variance partitioning
VP <- computeVariancePartitioning(mod, group = c(1,1,2,3,3,4,5,6,7,8,9,10), groupnames = c("Eelgrass LAI" ,"Temp range" ,"Mean temperature", "Mean nitrates" ,"Max current velocity" , "Mean salinity" , "Mean dissolved O2", "Macroalgae biomass", "Detritus biomass", "Eelgrass shoot density")) 

plotVariancePartitioning(mod, VP = VP)

VP.df <- as.data.frame(VP$vals) %>% 
  mutate(effect = factor(c("Eelgrass LAI (0.7%)", "Temp range (17.4%)","Mean temperature (11.1%)",  "Mean nitrates (9.4%)","Max current velocity (10%)","Mean salinity (5.7%)", "Mean dissolved O2 (14.8%)", "Macroalgae biomass (1.1%)","Detritus biomass (0.7%)","Eelgrass shoot density (2%)", "Random: Quadrat (8.2%)","Random: Site (6.7%)", "Random: Region (3.2%)","Random: Spatial distance (9%)"), levels = rev(c("Eelgrass LAI (0.7%)", "Temp range (17.4%)","Mean temperature (11.1%)",  "Mean nitrates (9.4%)","Max current velocity (10%)","Mean salinity (5.7%)", "Mean dissolved O2 (14.8%)", "Macroalgae biomass (1.1%)","Detritus biomass (0.7%)","Eelgrass shoot density (2%)", "Random: Quadrat (8.2%)","Random: Site (6.7%)", "Random: Region (3.2%)","Random: Spatial distance (9%)")), ordered = TRUE)) 

target <- c("Temp range (17.4%)","Mean temperature (11.1%)",  "Mean nitrates (9.4%)","Max current velocity (10%)","Mean salinity (5.7%)", "Mean dissolved O2 (14.8%)", "Macroalgae biomass (1.1%)","Detritus biomass (0.7%)","Eelgrass LAI (0.7%)", "Eelgrass shoot density (2%)", "Random: Quadrat (8.2%)","Random: Site (6.7%)", "Random: Region (3.2%)","Random: Spatial distance (9%)") # re-ordering factors so all biotic/biometrics are together for ease of looking

VP.df$effect <- VP.df[match(target, VP.df$effect),]

VP.df$effect <- c("Abiotic: Temp range (16%)","Abiotic: Mean temperature (14%)",  "Abiotic: Mean nitrates (8.9%)","Abiotic: Max current velocity (11%)","Abiotic: Mean salinity (3.6%)", "Abiotic: Mean dissolved O2 (14%)", "Biotic: Macroalgae biomass (1%)","Biotic: Detritus biomass (1.2%)","Biotic: Eelgrass shoot density (1.8%)", "Biotic: Eelgrass LAI (0.58%)", "Random: Quadrat (13%)","Random: Site (9.8%)", "Random: Region (4.9%)") # re-ordering factors so all biotic/biometrics are together for ease of looking
...
ggplot(VP.df1, aes(y = Variance, x = taxon, fill = factor(effect, levels = c("Temp range (17.4%)","Mean temperature (11.1%)",  "Mean nitrates (9.4%)","Max current velocity (10%)","Mean salinity (5.7%)", "Mean dissolved O2 (14.8%)", "Macroalgae biomass (1.1%)","Detritus biomass (0.7%)","Eelgrass LAI (0.7%)", "Eelgrass shoot density (2%)", "Random: Quadrat (8.2%)","Random: Site (6.7%)", "Random: Region (3.2%)","Random: Spatial distance (9%)"))))+
  geom_bar(stat = "identity", color = 1)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 75, hjust = 1))+
  scale_fill_manual(values = c("#F0F921FF","#F0F921FF","#F0F921FF","#F0F921FF","#F0F921FF","#F0F921FF", "#FCA636FF","#FCA636FF","#FCA636FF","#FCA636FF", "#E16462FF" ,"#B12A90FF" ,"#6A00A8FF" , "#0D0887FF"), name = "Mean variance explained across all species")+
...
```{r}
# Species associations/ corrplots
...
#### extract 10 most abundant from toplots
quad_cor <- toPlot[c(11,26,28,5,12,16,27,2,23,21),c(11,26,28,5,12,16,27,2,23,21)]
site_cor <- toPlot2[c(11,26,28,5,12,16,27,2,23,21),c(11,26,28,5,12,16,27,2,23,21)]
reg_cot <- toPlot3[c(11,26,28,5,12,16,27,2,23,21),c(11,26,28,5,12,16,27,2,23,21)]
...
corrplot(reg_cot, method = "color",col=colorRampPalette(c("#21908C","white","#440154"))(200), order = "hclust" , hclust.method = "ward.D2", tl.cex=.75, tl.col="black")
...
corrplot(reg_cot, method = "color",col=colorRampPalette(c("#21908C","white","#440154"))(200), order = "hclust" , hclust.method = "ward.D2", tl.cex=.3, tl.col="black", title=paste("random effect level:", mod$rLNames[3]), mar=c(0,0,1,0))
...
#distance-decay
library(reshape2)
distframe <- melt(as.matrix(distmat), varnames = c("row", "col"))
