...
Y <- read.csv("Y_matrix.csv", header = TRUE, stringsAsFactors = FALSE) # Site-by-sp matrix

Y <- read.csv(text=getURL("https://raw.githubusercontent.com/keilast/HMS-Seagrass/master/Data/Y_matrix.csv"), header = TRUE)
...
Pi <- read.csv("pi.csv", header = TRUE, stringsAsFactors = FALSE) #factors representing random effects from spatially nested sampling design
...
### CREATE HMSCdata OBJECT WITH THE ABOVE 4 INPUT MATRICES, GENERATE MODEL 
...
## Generate the hierarchical joint species distribution model with MCMC. We used 200000 interations, as the developers of the method note that Poisson distributions sometimes require 10-100 times more iterations to accurately estimate the model's parameters. The Markov Chain burn-in length is 1000, and is thinned to retain every 100th sample of the posterior distribution. 
model <- hmsc(hmsc1, family = "poisson", niter = 200000, nburn = 1000,
              thin = 100)
envmod <-  hmsc(envonly, family = "poisson", niter = 200000, nburn = 1000,
                thin = 100)
spacemod <-  hmsc(spaceonly, family = "poisson", niter = 200000, nburn = 1000,
                  thin = 100)
...
### Produce mixing, trace, density plots to look at model performance
...
### COMPUTE MODEL FIT (D2adj or Explained Deviance)
### Values shown in Table S2: Supporting Information
...
# This version of the function does the same thing but returns a vector of D2 values (one for each species) rather than one mean community-level value
...
D2.comm <- D2.avg.Poisson(Y, pred.hmsc) #single average community level value
D2 <- D2.Poisson(Y, pred.hmsc) #value for each species

plot(x=pre,y=D2,xlab="Average quadrat-level abundance",ylab="Predictive power (D2)") ## just as an example, can plot against abundance to visualize the relationship between D2 for each spp if desired
```
...
#### VARIATION PARTITIONING OF SPATIAL RANDOM EFFECTS AND ENVIRONMENTAL VARIABLES ----------
# Full Variation partitioning calculations (as summarized in Fig. S3, Supporting Info)
...
corrplot(averageCor, method = "color", col = colorRampPalette(c("#21908C","white","#440154"))(200), order = "hclust", hclust.method = "ward.D2", tl.col = "black", tl.cex = 0.8)
averageCor # incl all species (S6 Supplementary Material)
...
averageCor # incl top 20 species (Fig. 4a)
```
